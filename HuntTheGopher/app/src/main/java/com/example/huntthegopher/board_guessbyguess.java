...
public class board_guessbyguess extends AppCompatActivity {

    GameLogic game;
    Thread blue;
    Thread red;
    private Handler bHandler = new Handler() ;
    public boolean allowNextMove;
    ...
    public static final String TAG = "board_guessbyguess";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_board_guessbyguess);
        ...
        blue = new Thread(new board_guessbyguess.BlueThread());
        blue.start();

        red = new Thread(new board_guessbyguess.RedThread());
        red.start();
   ...
   /* try {
                ...
                for(int currY = 0; currY < 10; currY++) {
                    ...
                    synchronized (game) {
                        ...
                        if (game.getIsWon()) {
                            ...
                            //Log.i(TAG, "blue run: in else");
                            Move response1;
                            while(true) {
                                Log.i(TAG, "red run: in while");
                                if(allowNextMove) {
                                    response1 = game.playMove(currX, currY, Cell.Red);
                                    allowNextMove = false;

                                    final Move response = response1;
                ...
                for(int currX = 0; currX < 10; currX++) {
                    ...
                    synchronized (game) {
                        ...
                        if (game.getIsWon()) {
                            ...
                            Move response1;
                            while(true) {
                                Log.i(TAG, "blue run: in while");
                                if(allowNextMove) {
                                    response1 = game.playMove(currX, currY, Cell.Blue);
                                    allowNextMove = false;
    ...
    public void nextBtnHandler(View v) {
        allowNextMove = true;
        Log.i(TAG, "nextBtnHandler: " + allowNextMove);
    }
}